...
Wave 1 is live on Midnight Preprod. Wave 2 hardens the same protocol. Wave 3 targets Mainnet after an explicit compatibility gate. Explorer hashes are the settlement record. Judges can verify without a wallet via the hosted UI and public API.
...
3. **Constrained executor.** The agent ranks a K=3 private candidate set with Compact-identical helpers. It cannot authorize settlement.
...
MBBE proves the best eligible candidate among the K=3 private openings supplied to the executor. Matching is not MPC.
...
`auditRoot` `1bbc1cc2…`. UI: https://remit-front.vercel.app. API: https://remit-api-node.onrender.com. Persistence: Postgres ciphertext envelopes, RLS deny-all, no service role in the browser. Wallet reconnect uses connector `connect(networkId)` plus hashed tab vaults. Seven impure circuits. Local tests via `npm test`. `npm run judge:proof` reprints committed hashes with explorer URLs.

In-browser Compact circuits (1AM `getProvingProvider`): createMandate, revokeMandate, deposit, withdraw. Executor `fill` proving stays on operator-controlled proof infrastructure. Compact remains the settlement authority. 1AM is the recommended Preprod wallet path. Lace is an alternative; a local proof-server may be required.
...
Wave 3 targets a real Mainnet deployment after a compatibility gate: ledger, Compact, midnight-js, wallet SDK, connector, proof-server, indexer, DUST, proving keys, custody assets, and browser wallets. Production custody must separate testnet quote from Mainnet assets. Nothing ships to Mainnet on untested versions. Public observability will show solvency and receipts, never the private book.
...
Path B, compile: source of truth is `CONTRACT/src/remit_pool.compact` and `CONTRACT/src/remit_quote.compact`. Compact 0.31.1 / language 0.23. Fast gate: `npm run compile:skip-zk`. Judge acceptance: `compact compile` without `--skip-zk` (Windows Compact in WSL; Node/npm on Windows). Full compile emits 7 pool verifiers, provers, ZKIR, JS. No secret file. Then `npm test` and `npm run judge:proof`.

Path C, Chrome Preprod: connect 1AM (recommended), seal a mandate, confirm indexer `activeMandates`. Lace is an alternative wallet.

## Stack

Node 22. Compact `remit_pool.compact` (deposit, withdraw, placeOffer, cancelOffer, createMandate, revokeMandate, fill) and `remit_quote.compact`. Fastify API. Next.js workspace. Apache-2.0. The agent is constrained execution infrastructure: it may rank, it may not settle. Full architecture, diagrams, explorer links, and copy-paste compile commands: README.md. Roadmap: PLAN.md.
