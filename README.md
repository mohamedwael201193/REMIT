...
The executor is a constrained broker: it sees mandate and offer openings it is given. Matching is not MPC and is **not** global-book best execution. Compact `fill` proves the selected candidate is the unique best *eligible* opening among the K=3 private slots supplied in that transaction. Padding copies are `live=false` and cannot win. Deposit/withdraw amounts are public. `fill` is proven on the executor Node + local proof-server 8.1.0. Hosted Render does **not** run proof-server 8.1.0 (`httpSubmit` stays false). Render persists RFQ/mandate/audit **ciphertext** in Postgres (Supabase); the database never stores plaintext mandate terms, prices, sizes, or keys. `/agent/rank` returns a `selectedId` without openings; it does not prove or submit.
