...
import { publishPublicEvidence, readCommittedEvidence, readLocalLifecycleEvidence } from "./lib/public-evidence.ts";
...
const body = readCommittedEvidence(root) ?? readLocalLifecycleEvidence(root);
if (!body) throw new Error("apps/api/preprod-evidence.json and deployments/lifecycle.json missing");
